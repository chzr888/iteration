package com.chenzr.iteration.impl;

import java.util.HashMap;
import java.util.Map;

import com.chenzr.iteration.IterationContext;
import com.greenpineyu.fel.context.FelContext;
import com.greenpineyu.fel.context.Var;

/**
 * 迭代运算上下文实现
 * @author chenzr
 *
 */
public class IterationContextImpl extends HashMap<String, Var> implements FelContext , IterationContext {
	
	private static final long serialVersionUID = 1L;

	public IterationContextImpl() {
	}
	
	public IterationContextImpl(Map<String,Object> map){
		set(map);
	}

	@Override
	public Object get(String name) {
		Var object = super.get(name);
		if (object != null) {
			// 对象有值，或者包含此变量时，返回object
			return object.getValue();
		}
		return null;
	}
	
	@Override
	public void set(String name, Object value) {
		// 如果变量已经存在，就不再重复创建变量。
		Var var = getVar(name);
		if (var != null) {
			var.setValue(value);
		} else {
			this.put(name, new Var(name, value));
		}
	}

	public static String toString(Object var) {
		return var == null ? null : var.toString();
	}

	@Override
	public Var getVar(String name) {
		return super.get(name);
	}

	@Override
	public void setVar(Var var) {
		super.put(var.getName(), var);
	}

	@Override
	public void set(Map<String, Object> map) {
		if(map != null){
			for (Map.Entry<String, Object> e : map.entrySet()) {
				String name = e.getKey();
				Object value = e.getValue();
				this.set(name, value);
			}
		}
	}
	
}
